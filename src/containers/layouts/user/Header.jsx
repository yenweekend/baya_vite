...
const Header = ({ setMarginTop, setShowBanner, showBanner }) => {
  const [openHamburger, setOpenHamburger] = React.useState(false);

  const headerRef = useRef(null);
  useEffect(() => {
    if (headerRef.current) {
      setMarginTop(headerRef.current.getBoundingClientRect().height);
    }
  }, [showBanner]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <button
          className="center-y right-[8px] w-6 h-6 flex items-center justify-center"
          onClick={() => {
            setShowBanner(false);
          }}
        >
      ...
      <div className="bg-primary ">
        <div className=" flex items-center justify-between  2md:py-[10px] md:px-[15px] max-w-[1400px] mx-auto header-wrapper px-[6px]">
          <DropdownScreenUT
            open={openHamburger}
          ...
          >
            ...
            <DropdownScreenUTContent className={"top-[calc(100%-72px)]"}>
