const navigationComponents = [
  {
    title: "Sofa - Ghế thư giãn",
    link: "/categories/sofa-ghe-thu-gian",
    subNav: [
      { title: "Sofa", link: "/categories/sofa" },
      { title: "Ghế thư giãn", link: "/categories/ghe-thu-gian" },
      { title: "Đệm ngồi", link: "/categories/dem-ngoi" },
    ],
  },
  {
    title: "Bàn",
    link: "/categories/ban",
    subNav: [
      { title: "Bàn ăn", link: "/categories/ban-an" },
      { title: "Bàn trà, cà phê", link: "/categories/ban-tra-ca-phe" },
      { title: "Bàn làm việc", link: "/categories/ban-lam-viec" },
      { title: "Bàn học", link: "/categories/ban-hoc" },
      { title: "Bàn trang điểm", link: "/categories/ban-trang-diem" },
      { title: "Bàn góc", link: "/categories/ban-goc" },
      { title: "Bàn ngoài trời", link: "/categories/ban-ngoai-troi" },
      { title: "Bộ bàn ghế", link: "/categories/bo-ban-ghe" },
    ],
  },
  {
    title: "Ghế",
    link: "/categories/ghe",
    subNav: [
      { title: "Ghế ăn", link: "/categories/ghe-an" },
      { title: "Ghế đôn", link: "/categories/ghe-don" },
      { title: "Ghế văn phòng", link: "/categories/ghe-van-phong" },
      { title: "Ghế ngồi học", link: "/categories/ghe-ngoi-hoc" },
      { title: "Ghế trẻ em", link: "/categories/ghe-tre-em" },
      { title: "Ghế thư giãn", link: "/categories/ghe-thu-gian" },
    ],
  },
  {
    title: "Giường nệm",
    link: "/categories/giuong-nem",
    subNav: [
      { title: "Giường", link: "/categories/giuong" },
      { title: "Nệm", link: "/categories/nem" },
      { title: "Nệm Yoga", link: "/categories/nem-yoga-1" },
    ],
  },
  {
    title: "Chăn ga gối",
    link: "/categories/chan-ga-goi",
    subNav: [
      { title: "Chăn", link: "/categories/chan" },
      { title: "Ga", link: "/categories/ga" },
      { title: "Gối ngủ", link: "/categories/goi-ngu" },
      { title: "Gối tựa", link: "/categories/goi-tua" },
      { title: "Gối ôm", link: "/categories/goi-om" },
      { title: "Gối tập yoga", link: "/categories/goi-yoga" },
    ],
  },
  {
    title: "Tủ kệ",
    link: "/categories/tu-ke",
    subNav: [
      { title: "Kệ sách", link: "/categories/ke-sach" },
      { title: "Tủ quần áo", link: "/categories/tu-quan-ao" },
      { title: "Tủ kệ Tivi", link: "/categories/tu-tivi" },
      { title: "Tủ đầu giường", link: "/categories/tu-dau-giuong" },
      { title: "Tủ kệ giày", link: "/categories/tu-ke-giay" },
      { title: "Tủ ngăn kéo", link: "/categories/tu-ngan-keo" },
    ],
  },
  {
    title: "Nội thất văn phòng",
    link: "/categories/noi-that-van-phong",
    subNav: [
      { title: "Ghế văn phòng", link: "/categories/ghe-van-phong" },
      { title: "Dụng cụ văn phòng", link: "/categories/van-phong-pham" },
    ],
  },
  {
    title: "Trang trí",
    link: "/categories/trang-tri-nha-cua",
    subNav: [
      { title: "Lọ hoa - Bình hoa", link: "/categories/lo-hoa" },
      { title: "Gương treo tường", link: "/categories/guong-treo-tuong" },
      { title: "Gương đứng", link: "/categories/guong-dung" },
      { title: "Nến thơm", link: "/categories/nen-thom" },
      { title: "Đồng hồ báo thức", link: "/categories/dong-ho-bao-thuc" },
      { title: "Tiện ích sắp xếp", link: "/categories/tien-ich-sap-xep" },
    ],
  },
  {
    title: "Nhà bếp",
    link: "/categories/phu-kien-nha-bep-1",
    subNav: [
      { title: "Tô - Chén - Đĩa", link: "/categories/to-chen-dia" },
      { title: "Cốc & Ly", link: "/categories/coc-ly" },
      { title: "Ấm trà - Bộ ấm trà", link: "/categories/am-tra-bo-am-tra" },
      { title: "Dụng cụ nấu nướng", link: "/categories/dung-cu-nau-nuong" },
      {
        title: "Giá đỡ - Dụng cụ đựng",
        link: "/categories/gia-do-dung-cu-dung",
      },
      { title: "Khăn trải bàn", link: "/categories/khan-trai-ban" },
      { title: "Găng tay - Tạp dề", link: "/categories/gang-tay-tap-de" },
    ],
  },
  {
    title: "Phòng tắm",
    link: "/categories/phu-kien-phong-tam",
    subNav: [
      { title: "Thảm phòng tắm", link: "/categories/tham-phong-tam" },
      { title: "Rèm tắm", link: "/categories/rem-tam" },
      { title: "Vật dụng phòng tắm", link: "/categories/dung-cu-phong-tam" },
    ],
  },
];

export default navigationComponents;
